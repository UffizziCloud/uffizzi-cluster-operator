...
start-test-minikube-tainted: ## Start a minikube cluster with a tainted node for testing.
	...
	kubectl get statefulsets --all-namespaces -o jsonpath="{range .items[*]}{.metadata.namespace}{' '}{.metadata.name}{'\n'}{end}" | while read -r namespace statefulset; do kubectl patch statefulset $(statefulset) --patch-file="./.github/kubectl-patch/nodeselector-toleration.yaml" -n $(namespace); done
